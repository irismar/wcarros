...
<form action="?destinatario=<?php echo $_GET['destinatario']; ?>&&id=<?php echo $_GET['id']; ?>&&id_membro=<?php echo $_GET['id_membro']; ?>&&segure=<?php echo $_GET['segure']; ?>&&remetente=<?=$_GET['remetente']?>&&id_remetente=<?=$_GET['id_remetente']?>&&telefone=<?=$_GET['telefone']?>&&foto=<?=$_GET['foto']?>&&endereco1=<?=$_GET['endereco1']?>" method="post" >
...
<? if(isset($_GET['destinatario']) && ( isset($_POST['memsagem'])&& ($_POST['memsagem']!='')) ) { 
  ...
  $sql="INSERT INTO propostas (remetene,Destinatario,mensagem,id_estoque,email,data,foto,endereco,cod_seguranca,alerta,id_membro,marca,modelo,preco,id_facebook,resposta,id_remetente) VALUES ('".$_GET['remetente']."', '".@$_GET['destinatario']."','".$_POST['memsagem']."','".$_GET['id']."','".$_GET['telefone']."','". date('Y-m-d H:i:s')."','".@$_GET["foto"]."','".@$_GET['endereco1']."','".$_GET['segure']."','+1','".trim($_GET['id_membro'])."','".@trim($_GET['marca'])."','".@trim($_GET['modelo'])."','".@trim($_GET['preco'])."','".@trim($_SESSION['id_face'])."','1','".@trim($_GET['id_remetente'])."')";
